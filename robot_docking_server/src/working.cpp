...
private:
        ...
        const std::shared_ptr<DockingGoalHandle> goal_handle)
    {
        ...
        while (rclcpp::ok()) 
        {
            ...
            // Send the feedback
            // RCLCPP_INFO(this->get_logger(), "offsets: X: %f, Y: %f", x_offset_, y_offset_);
    ...
    void publish_2d_goal_pose()
    {
        auto goal_msg = geometry_msgs::msg::PoseStamped();
        goal_msg.header.stamp = this->now();
        goal_msg.header.frame_id = "map";

        goal_msg.pose.position.x = -1.51;
        goal_msg.pose.position.y = -0.27;
        goal_msg.pose.orientation.x = 0.0;
        goal_msg.pose.orientation.y = 0.0;
        goal_msg.pose.orientation.z = 0.62;
        goal_msg.pose.orientation.w = 0.78; // Assuming no rotation
